...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($attendanceData)) {
        foreach ($attendanceData as $data) {
            ...
            // Send email if student is marked as absent
            if ($attendanceStatus === 'absent') {
                $studentQuery = "SELECT emailAddress FROM tblstudents WHERE registrationNumber = '$studentID'";
                $result = $conn->query($studentQuery);
                if ($result->num_rows > 0) {
                    $student = $result->fetch_assoc();
                    $to = $student['emailAddress'];
                    $subject = "Absence Notification";
                    $message = "Dear Student,\\n\\nYou have been marked absent for the course $course on $date.\\n\\nBest regards,\\nYour University";
